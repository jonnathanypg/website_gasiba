...
contactForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const response = await fetch('contac.php', {
            method: 'POST',
            body: formData
        });
        ...
        if (result.type === 'success') {
            formStatus.innerHTML = `<div style="color: var(--color-success); margin-top: 1rem; padding: 1rem; background: rgba(67, 233, 123, 0.1); border-radius: 10px; border: 1px solid var(--color-success);"><i class="fas fa-check-circle"></i> ${result.message}</div>`;
            contactForm.reset();
        } else {
            throw new Error(result.message);
        }
    } catch (error) {
        formStatus.innerHTML = `<div style="color: #f5576c; margin-top: 1rem; padding: 1rem; background: rgba(245, 87, 108, 0.1); border-radius: 10px; border: 1px solid #f5576c;"><i class="fas fa-exclamation-circle"></i> Error: ${error.message || 'Hubo un problema al enviar el mensaje.'}</div>`;
    } finally {
